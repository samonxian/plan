...
//        $d['type'] = $chosen['type'][$d['type']];
        ...
        if($open['d']!=date('d'))
            ...
            $d['open'] = "今天 ".$time2[0].':'.$time2[1];
            if($time2[0]==date('H')){
                $t['$color'] = $color;
                $baseUrl = Yii::app()->baseUrl;
                $t['$img'] = '<img src="'.$baseUrl.'/kaifu_files/hot.gif"></a>';
            }
        }
        if(isset($d['type']))
            $d['type'] = Tvar::$gameRelative[$d['type']];
        if($d['serviceName']=='')
            $d['service'] = "双线{$d['service']}区";
        else $d['service'] = $d['serviceName'];
        $t['$hour'] = $time2[0];
        $t['$gift'] = '';
        $t['$activity'] = '-';
        $d['registerAddress'] = Yii::app()->createUrl('stage/progress',array("id"=>$d['id']));
