...
class NGridView extends CGridView {
    ...
    public $summaryText  = '<h4 class="mt10 absolute" style="right:20px;top:-50px;">共 {count} 记录</h4>';
    public $summaryCssClass  = 'relative';
    public $itemsCssClass = 'table table-striped table-bordered table-hover';
    public $emptyText = '没有找到相关数据！';
    
    public $pager = array('class'=>'ext.widgets.NPager');
    ...
    protected function initColumns() {
        ...
        foreach ($this->columns as $i => $column) {
            if (is_array($column) && !isset($column['class']))
                $this->columns[$i]['class'] = 'ext.widgets.NDataColumn';
        }
