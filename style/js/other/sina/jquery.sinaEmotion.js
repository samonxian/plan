...
$(function() {
	...
	$.ajax( {
		...
		success : function(response) {
			...
			if(sinaAfter!=undefined) sinaAfter();
		}
	});
});
...
function AnalyticEmotion(s) {
	if(typeof (s) != "undefined") {
		var reg = /\[(.*?)\]/g;
		var sArr;
		while(sArr = reg.exec(s)){
			if(uSinaEmotionsHt.containsKey(sArr[0])) {
				var reStr = "<img title=\""+sArr[1]+"\" src=\"" + uSinaEmotionsHt.get(sArr[0]) + "\" height=\"22\" width=\"22\" />";
				s = s.replace(sArr[0], reStr);
			}
		}
	}
	return s;
...
(function($){
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			var eTop = $(this).offset().top + 30;
			var eLeft = $(this).offset().left - 1;
